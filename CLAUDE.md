...
- **Native app login gate** (mobile only): every cold launch of the Capacitor app, if no customer is signed in, the layout redirects to `/account/login?firstLaunch=true&redirectTo=<path>`. Source of truth for "is signed in" is the customer JWT in `localStorage["medusa_auth_token"]` (via `useCustomer()`). The "Skip for now" tertiary link on the login page writes a **session-scoped** suppression flag at `sessionStorage["suprameds_native_login_skipped"]` — survives intra-session navigation and warm resume from background, but Capacitor's webview clears sessionStorage on process termination, so the next cold open re-prompts. To re-trigger for testing, log out (clears `medusa_auth_token`) and kill+relaunch the app. The legacy persistent flag `localStorage["suprameds_native_login_resolved"]` is auto-cleaned by a migration effect in `apps/storefront/src/components/layout.tsx`; do not reintroduce it.
