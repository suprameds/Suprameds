/**
 * Fix migration v2: add stock levels + batches to the 10 test products.
 *
 * 005 created products but failed on inventory (SKU conflict).
 * 006 had the fix but failed on LinkModel query traversal.
 * This script (007) uses the corrected two-step query and will
 * THROW on failure so Medusa retries on next deploy.
 */
...
const logger = createLogger("migration:007-fix-test-product-stock-v2")
...
export default async function fixTestProductStockV2({
  container,
}: {
  ...
  if (!stockLocation?.id) {
    logger.error("007: No stock location found — skipping.")
    return
  }
  ...
  for (const product of PRODUCTS) {
    try {
      ...
      if (!productRecord?.id) {
        logger.warn(`007: product "${product.handle}" not found — skipping.`)
        skipped++
        continue
      }
      ...
      if (!variant?.id || !invItemId) {
        logger.warn(`007: no variant/inventory for "${product.handle}" — skipping.`)
        skipped++
        continue
      }
      ...
      for (const batch of product.batches) {
        try {
          ...
          await batchService.createBatches({
            ...
            status: "active",
            metadata: { source: "seed:007-fix-v2" },
          })
          batchesCreated++
          logger.info(`007:   ↳ ${batch.lotNumber} (qty: ${batch.qty}, exp: ${batch.expiryDate})`)
        } catch (bErr: any) {
          logger.warn(`007:   batch ${batch.lotNumber} failed: ${bErr.message}`)
        }
      }
      ...
      logger.info(
        `007: FIXED "${product.handle}" — ${totalQty} units, ${batchesCreated} new batches`
      )
      fixed++
    } catch (err: any) {
      logger.error(`007: ERROR "${product.handle}": ${err.message}`)
      errors++
    }
  }

  logger.info(`007: done. Fixed: ${fixed}, skipped: ${skipped}, errors: ${errors}`)

  // Throw if any products failed so Medusa retries on next deploy
  if (errors > 0) {
    throw new Error(`007-fix-test-product-stock-v2: ${errors} product(s) failed — will retry on next deploy`)
  }
}
