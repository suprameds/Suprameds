...
/**
 * POST /store/pharmacist/prescriptions/:id/create-order
 * Pharmacist creates a COD order on behalf of a customer from an approved prescription.
 */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (rx.status !== "approved") {
    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Prescription must be approved. Current: ${rx.status}`)
  }
  if (!rx.customer_id) {
    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Customer account required for order creation.")
  }

  // 2. Block Schedule X / narcotic / cold chain
  ...
  try {
    ...
    throw new MedusaError(MedusaError.Types.NOT_FOUND, `Customer ${rx.customer_id} not found.`)
  }
  ...
  if (!shippingAddress) {
    throw new MedusaError(MedusaError.Types.INVALID_DATA, "No shipping address available.")
  }
  ...
  try {
    ...
    // 8. Create payment collection + COD session
    ...
    try {
      await prescriptionService.updatePrescriptions({ id: prescriptionId, status: "used" })
    } catch (err) {
      logger.warn(`Failed to update Rx status: ${(err as Error).message}`)
    }
    ...
    try {
      ...
      logger.warn(`Failed to emit event: ${(err as Error).message}`)
    }

    return res.status(201).json({ order_id: orderId, prescription_id: prescriptionId, message: "Order created successfully." })
  } catch (err) {
    logger.error(`Order creation failed for Rx ${prescriptionId} (cart ${cart.id}): ${(err as Error).message}`)
