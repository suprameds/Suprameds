...
/**
 * GET /admin/products/search?q=paracetamol&limit=20&offset=0
 *
 * Admin product search with variant + pricing data for pharmacist order creation.
 * Reuses PostgreSQL FTS from store search, adds variant IDs and INR prices.
 */
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  const searchQuery = `
    ...
    SELECT
      ...
      pv."sku",
      pma."amount" AS price_amount,
      pma."currency_code" AS price_currency
    FROM ranked r
    LEFT JOIN LATERAL (
      SELECT v."id", v."title", v."sku"
  ...
  try {
    const result = await pgConnection.raw(searchQuery, {
    ...
    req.scope
      ...
      .error("[admin:products:search] FTS query failed:", error)
